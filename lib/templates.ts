...
export const templates: TemplateDefinition[] = [
  ...
  {
    key: "city-inspiration",
    name: "城市灵感",
    eyebrow: "4 LIVE APIS",
    description: "城市定位 → 实时天气，并组合一言与随机动漫插图。",
    icon: "☀",
    accent: "#ff7657",
    config: {
      name: "今日城市灵感",
      description: "四个免费公开 API 编排成一张会随城市变化的动态卡片",
      template: "city-inspiration",
      inputs: [{
        key: "city", label: "城市", type: "string", required: true,
        defaultValue: "上海", previewValue: "上海",
      }],
      requests: [
        {
          id: "location", name: "解析城市坐标", type: "http",
          url: "https://geocoding-api.open-meteo.com/v1/search",
          query: { name: "{{input.city}}", count: "1", language: "zh", format: "json" },
          failureMode: "abort",
        },
        {
          id: "weather", name: "查询实时天气", type: "http",
          url: "https://api.open-meteo.com/v1/forecast",
          query: {
            latitude: "{{requests.location.results.0.latitude}}",
            longitude: "{{requests.location.results.0.longitude}}",
        ...
        {
          id: "quote", name: "获取今日一言", type: "http",
          url: "https://v1.hitokoto.cn/",
          query: { encode: "json" },
          failureMode: "continue",
        },
        {
          id: "art", name: "获取动漫插图", type: "http",
      ...
      mapping: {
        title: bind("requests.location.results.0.name", "你的城市"),
        subtitle: bind("requests.location.results.0.country", "今日城市灵感"),
        badge: bind("requests.weather.current_units.temperature_2m", "°C"),
        description: bind("requests.quote.hitokoto", "给今天留一点灵感。"),
        background: bind(""),
        url: bind("requests.art.results.0.source_url", "https://hitokoto.cn/"),
        stats: [],
      },
      layout: { blocks: [
        {
          id: "hero", type: "hero", hidden: false, align: "left",
          title: bind("requests.location.results.0.name", "你的城市"),
          subtitle: bind("requests.location.results.0.admin1", "今日城市灵感"),
          badge: { path: "requests.weather.current.temperature_2m", formatters: [{ type: "suffix", value: "°C" }] },
        },
        {
          id: "weather", type: "stats", hidden: false, columns: 4,
          items: [
            { label: "当前温度", value: { path: "requests.weather.current.temperature_2m", formatters: [{ type: "suffix", value: "°C" }] } },
            { label: "体感温度", value: { path: "requests.weather.current.apparent_temperature", formatters: [{ type: "suffix", value: "°C" }] } },
            { label: "相对湿度", value: { path: "requests.weather.current.relative_humidity_2m", formatters: [{ type: "suffix", value: "%" }] } },
            { label: "风速", value: { path: "requests.weather.current.wind_speed_10m", formatters: [{ type: "suffix", value: " km/h" }] } },
          ],
        },
        {
          id: "quote", type: "text", hidden: false, label: "今日一言",
          content: bind("requests.quote.hitokoto", "给今天留一点灵感。"),
        },
        {
          id: "source", type: "text", hidden: false, label: "出处",
          content: { path: "requests.quote.from", fallback: "一言", formatters: [{ type: "prefix", value: "— " }] },
        },
        {
          id: "art", type: "image", hidden: false, ratio: "wide", alt: "今日动漫插图",
          src: bind("requests.art.results.0.url"),
        },
        {
          id: "artist", type: "text", hidden: false, label: "ILLUSTRATION",
          content: bind("requests.art.results.0.artist_name", "NEKOSBEST"),
        },
        {
          id: "links", type: "links", hidden: false,
          items: [
            { label: "查看插图来源", url: bind("requests.art.results.0.source_url", "https://nekos.best/"), style: "primary" },
            { label: "关于一言", url: bind("requests.__hitokoto", "https://hitokoto.cn/"), style: "secondary" },
          ],
        },
      ] },
      theme: {
