...
当前版本提供 B 站创作者、GitHub 开发者、牛客档案、知乎创作者、力扣进度、城市灵感、空白 API、多源人物档案和指标仪表盘九种起点：
...
“城市灵感”是一个完整的四接口示例：先用 Open-Meteo Geocoding 将城市名解析为经纬度，再把前序响应传给天气接口，同时获取一言与 NEKOSBEST 插图。它不包含写死的展示数据，输入城市变化后会重新执行整条工作流。
